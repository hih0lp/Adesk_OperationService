...
import Adesk_OperationService.Constants.OperationType;
...
public class RequestController {
        ...
        @Transactional
        @Operation(summary = "Создание нового запроса с файлами", description = "Создает новый запрос на операцию с прикрепленными файлами. Требуется право CREATE_REQUEST_AND_DELETE_BEFORE_APPROVE")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Скачивание файла", description = "Скачивает файл по его ID. Требуется право REQUEST_WORK")
        @ApiResponses(value = {
        ...
        public CompletableFuture<ResponseEntity<byte[]>> downloadFile(
                        @Parameter(description = "ID файла", required = true) @PathVariable Long id,
                        HttpServletRequest request) {
        ...
        @Transactional
        @Operation(summary = "Удаление запросов", description = "Удаляет несколько запросов по ID. Доступ зависит от прав пользователя")
        @ApiResponses(value = {
        ...
        public CompletableFuture<ResponseEntity<?>> deleteRequestsAsync(
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Список ID запросов для удаления", required = true, content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModelDeleteDTO.class)))) @RequestBody List<RequestModelDeleteDTO> dtos,
                        HttpServletRequest request) {
                var requests = _requestRepository
                                .findAllById(dtos.stream().map(x -> x.getId()).collect(Collectors.toList()));
                ...
                if (Arrays.stream(request.getHeader("X-User-Permissions").split(","))
                        ...
                        if (requests.stream().anyMatch(x -> x.getApprovedStatus() != RequestStatuses.APPROVING))
                                return CompletableFuture.completedFuture(ResponseEntity.badRequest()
                                                .body("you can delete only request with approving status"));

                        if (!Arrays.stream(request.getHeader("X-User-Permissions").split(","))
                                        .anyMatch(s -> s.equals("REQUEST_WORK")))
                                if (requests.stream().anyMatch(
                                                s -> !s.getCreatorEmail().equals(request.getHeader("X-User-Email"))))
                                        return CompletableFuture.completedFuture(ResponseEntity.badRequest()
                                                        .body("you can delete only yours request"));
                        ...
                        if (requests.stream().anyMatch(s -> s.getApprovedStatus() == RequestStatuses.APPROVING))
                                return CompletableFuture.completedFuture(ResponseEntity.badRequest()
                                                .body("you can only delete projects which approved"));

                        if (!Arrays.stream(request.getHeader("X-User-Permissions").split(","))
                                        .anyMatch(s -> s.equals("REQUEST_WORK")))
                                if (requests.stream().anyMatch(
                                                s -> s.getResponsibleManager() != request.getHeader("X-User-Email")))
                                        return CompletableFuture.completedFuture(ResponseEntity.badRequest()
                                                        .body("you can only delete your projects"));
        ...
        @Operation(summary = "Получение запросов компании", description = "Возвращает список запросов для текущей компании (статус APPROVING)")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
        ...
        public CompletableFuture<ResponseEntity<?>> getRequestsByProjectName(HttpServletRequest request) {
                return CompletableFuture.supplyAsync(() -> {
                        var requests = _requestRepository
                                        .findByCompanyId(Long.parseLong(request.getHeader("X-Company-Id")));
                        if (requests.isEmpty())
        ...
        @Transactional
        @Operation(summary = "Утверждение запроса", description = "Утверждает запрос с указанным ID. Требуются права REQUEST_WORK или APPROVE_REQUEST_AND_DELETE_AFTER_APPROVE")
        @ApiResponses(value = {
        ...
        public CompletableFuture<ResponseEntity<?>> approveRequest(
                        @Parameter(description = "ID запроса для утверждения", required = true) @PathVariable Long requestId,
                        HttpServletRequest request) {
                ...
                return CompletableFuture.supplyAsync(() -> {
                        ...
                        if (!Arrays.stream(request.getHeader("X-User-Permissions").split(","))
                                        .anyMatch(s -> s.equals("REQUEST_WORK")
                                                        || s.equals("APPROVE_REQUEST_AND_DELETE_AFTER_APPROVE")))
        ...
        @Transactional
        @Operation(summary = "Отклонение запроса", description = "Отклоняет запрос с указанным ID. Требуются права REQUEST_WORK или APPROVE_REQUEST_AND_DELETE_AFTER_APPROVE")
        @ApiResponses(value = {
        ...
        public CompletableFuture<ResponseEntity<?>> disapproveRequest(
                        @Parameter(description = "ID запроса для отклонения", required = true) @PathVariable Long requestId,
                        HttpServletRequest request) {
        ...
        @Operation(summary = "Получение запросов за сегодня", description = "Возвращает запросы текущей компании за сегодняшний день")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        public CompletableFuture<ResponseEntity<?>> getRequestsOrderByDateToday(HttpServletRequest request) {
                return CompletableFuture
                                .supplyAsync(() -> _requestRepository
                                                .findByCompanyId(Long.parseLong(request.getHeader("X-Company-Id"))))
        ...
        @Operation(summary = "Получение запросов за неделю", description = "Возвращает запросы текущей компании за текущую неделю")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение запросов за месяц", description = "Возвращает запросы текущей компании за текущий месяц")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение запросов по диапазону дат", description = "Возвращает запросы текущей компании в указанном диапазоне дат")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
        ...
        public CompletableFuture<ResponseEntity<?>> getRequestsOrderByDates(
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Диапазон дат для фильтрации", required = true, content = @Content(schema = @Schema(implementation = SortByDateDTO.class))) @RequestBody SortByDateDTO dto,
                        HttpServletRequest request) {
                ...
                return CompletableFuture
                                ...
                                .thenCompose(requests -> {
                                        ...
                                        return _timeService
                                                        .filterByDateTimeRangeAsync(approvingRequests, dto.date1,
                                                                        dto.date2)
                                                        .thenApply(x -> ResponseEntity.ok().body(x));
                                });
        }
        ...
        @Operation(summary = "Получение запросов за квартал", description = "Возвращает запросы текущей компании за указанный квартал")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
        ...
        public CompletableFuture<ResponseEntity<?>> getRequestsOrderByDateQuarter(
                        @Parameter(description = "Номер квартала (1-4)", required = true, example = "1") @PathVariable int numberOfQuarter,
                        HttpServletRequest request) {
        ...
        @Operation(summary = "Получение операций по проекту", description = "Возвращает утвержденные операции (запросы) для указанного проекта")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        })
        public CompletableFuture<ResponseEntity<?>> getProjectOperations(
                        @Parameter(description = "ID проекта", required = true) @PathVariable Long projectId,
                        HttpServletRequest request) {
        ...
        @Operation(summary = "Получение запросов за год", description = "Возвращает запросы текущей компании за текущий год")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Запросы успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение всех операций компании", description = "Возвращает все утвержденные операции (запросы) текущей компании")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        public CompletableFuture<ResponseEntity<?>> getCompanyOperations(HttpServletRequest request) {
                return CompletableFuture.supplyAsync(() -> {
                        var operations = _requestRepository
                                        .findByCompanyId(Long.parseLong(request.getHeader("X-Company-Id")));
                        if (operations.isEmpty())
                                return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
        ...
        @Operation(summary = "Получение операций компании за сегодня", description = "Возвращает утвержденные операции текущей компании за сегодняшний день")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение операций компании за неделю", description = "Возвращает утвержденные операции текущей компании за текущую неделю")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение операций компании за месяц", description = "Возвращает утвержденные операции текущей компании за текущий месяц")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение операций компании за квартал", description = "Возвращает утвержденные операции текущей компании за указанный квартал")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "400", description = "Неверный номер квартала"),
        ...
        @Operation(summary = "Получение операций компании по диапазону дат", description = "Возвращает утвержденные операции текущей компании в указанном диапазоне дат")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "400", description = "Невалидные даты"),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
        ...
        public CompletableFuture<ResponseEntity<?>> getCompanyOperationsOrderByDates(
                ...
                return CompletableFuture
                                ...
                                .thenCompose(operations -> {
                                        ...
                                        return _timeService
                                                        .filterByDateTimeRangeAsync(approvedOperations, dto.date1,
                                                                        dto.date2)
                                                        .thenApply(x -> ResponseEntity.ok().body(x));
                                });
        }
        ...
        @Operation(summary = "Получение операций компании за год", description = "Возвращает утвержденные операции текущей компании за текущий год")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операции успешно получены", content = @Content(array = @ArraySchema(schema = @Schema(implementation = RequestModel.class)))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        ...
        @Operation(summary = "Получение статистики по проекту", description = "Возвращает статистику по операциям указанного проекта")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Статистика успешно получена", content = @Content(schema = @Schema(implementation = StatDTO.class))),
                        @ApiResponse(responseCode = "204", description = "Нет данных"),
                        @ApiResponse(responseCode = "500", description = "Внутренняя ошибка сервера")
        })
        public CompletableFuture<ResponseEntity<?>> getProjectStatistic(
                ...
                return CompletableFuture.supplyAsync(() -> {
                        ...
                        stat.setRevenue(projectOperations.stream()
                                        .filter(x -> x.getTypeOfOperation().equals(OperationType.income))
                                        .mapToDouble(x -> x.getSum())
                                        .sum());

                        stat.setProfit(projectOperations.stream()
                                        .filter(x -> x.getTypeOfOperation().equals(OperationType.outcome))
                                        .mapToDouble(x -> x.getSum()) // предпосчитываем все то, что в минусе
                                        .sum());

                        stat.setProfit(stat.getRevenue() - stat.getProfit());
        ...
        @Transactional
        @Operation(summary = "Редактирование операции", description = "Обновляет существующую операцию по ID с новыми данными из формы")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Операция успешно отредактирована", content = @Content(schema = @Schema(implementation = String.class))),
        ...
        public CompletableFuture<ResponseEntity<?>> editOperationAsync(
                        @Parameter(description = "ID операции", required = true, example = "123") @PathVariable Long id,

                        @Parameter(description = "Данные для обновления операции", required = true) @ModelAttribute RequestFormDTO dto,
